...
int Inventory::searchInventory(std::vector<item> &inventory, const QString &itemName) const {
    auto it =
...
const item &Inventory::getInventoryItem(std::vector<item> &inventory, int index) const {
    return inventory[index];
}

void Inventory::addItem(std::vector<item> &inventory, const item itemToAdd, int itemIndex) {
    if (itemIndex == -1) {
        inventory.push_back(itemToAdd);
    } else {
        setItemQuantity(inventory, itemIndex, getItemQuantity(inventory, itemIndex) + 1);
    }
}
...
void Inventory::setItemEquipped(std::vector<item> &inventory, int index, int value) {
...
void Inventory::setItemQuantity(std::vector<item> &inventory, int index, int value) {
