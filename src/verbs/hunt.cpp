  ...
  if (huntingObj.getDailyHunts() < 2) {
    if (location->getName() == "The valley") {
      ...
      if (bowIndex != -1 && arrowIndex != -1) {
        ...
        switch (huntResult) {
        case 0:
            sfxPlayer.play("qrc:/audio/sfx/hunt.mp3", sfxPlayer.getdefSfxVol(), 0);
            mainWindow->setDescription(QString("I spotted %1 %2.")
                .arg(handlingObj.getArticle(target), target.toLower()));
            break;
        case 1:
            mainWindow->setDescription(QString("I was unable to find %1 %2 in the valley.")
                .arg(handlingObj.getArticle(target), target.toLower()));
            break;
        case 2:
            mainWindow->setDescription(QString("I thought of hunting %1, but it didn't seem reasonable in "
                    "the area.")
                .arg(target.toLower()));
            break;
        }
